...
export class ProjectManager {
  ...
  async detectProject(): Promise<ProjectInfo | null> {
    ...
    let projectName = gitInfo?.repo || folderName;
    if (gitInfo?.branch) {
      projectName = `${projectName}/${gitInfo.branch}`;
    }
   ...
   */
  private getGitRemoteInfo(): { remoteUrl: string; owner: string; repo: string; branch: string; } | null {
    try {
      ...
      const branch = headName ? headName : 'main';
